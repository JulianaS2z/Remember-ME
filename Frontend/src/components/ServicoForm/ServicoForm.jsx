...
export default function ServicoForm({ onSubmit, onCancel, initialData = null, loading = false }) {
  ...
  useEffect(() => {
    if (initialData) setForm({ nome: initialData.nome || '', duracao: initialData.duracao || '', preco: initialData.preco || '' })
    else setForm(EMPTY)
  }, [initialData])
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="number" name="duracao" value={form.duracao} onChange={handle} placeholder="120" className="rm-input" />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1.5">Preço (R$)</label>
          <input type="number" step="0.01" name="preco" value={form.preco} onChange={handle} placeholder="800.00" className="rm-input" />
        </div>
      </div>
